...
def post_process_object_detection(
    outputs, threshold=0.1, nms_threshold=0.3, target_sizes=None
):
    ...
    if target_sizes is not None:
        if len(logits) != len(target_sizes):
            raise ValueError(
                "Make sure that you pass in as many target sizes "
                "as the batch dimension of the logits"
            )
        if target_sizes.shape[1] != 2:
            raise ValueError(
                "Each element of target_sizes must contain "
...
class Owl_ViT:
    ...
    def __call__(
        ...
        with torch.cuda.device(self.device):
            ...
            if not self.is_v2:
                target_sizes = torch.Tensor([image.shape[:2] for image in images]).to(
                    self.device
                )
            else:
            ...
            if self.is_v2:
                ...
                for result in results:
                    result["boxes"][:, [0, 2]] = (
                        result["boxes"][:, [0, 2]] * orig_w / padded_w
                    )
                    result["boxes"][:, [1, 3]] = (
                        result["boxes"][:, [1, 3]] * orig_h / padded_h
                    )
